...
└── .cc-dev-framework/          ← 被拷贝到目标项目的框架目录
    ├── main.py                 ← 主入口
    ├── status.py               ← 进度查看
    ├── init.sh                 ← 依赖安装 + 冒烟测试（Executor 按需填写）
    ├── dev.sh                  ← 项目启动命令（Executor 按需填写）
    ├── cleanup.sh              ← 杀后台进程 + 释放文件锁（Executor 按需填写）
...
| 阶段 | 做什么 |
|------|--------|
| INIT | git init + 运行 init.sh（失败仅警告，不阻塞） |
...
**Python 可捕获的异常**（Claude 超时、连接断开等）：main.py 入口 `try/except` 捕获，退出前打印当前 feature + 恢复命令。Ctrl+C（SIGINT）同理。

**OS 级杀进程**（OOM、SIGKILL、TerminateProcess）：Python 无法捕获。通过 `.session_lock` 标记文件检测：
- `main()` 启动时写 `.session_lock`（含阶段、feature、时间）
- 正常退出 / Ctrl+C / sys.exit / Python 异常时删除
- 下次启动发现 `.session_lock` 还在 → 上次是被 OS 杀掉的，打印诊断信息（终止阶段、feature、可能原因）

用户只需重新运行 `python .cc-dev-framework/main.py` 即可从断点继续。

## init.sh, dev.sh 与 cleanup.sh

- `init.sh` — 依赖安装 + 冒烟测试。main.py 阶段 1 运行，失败仅警告不阻塞
- `dev.sh` — 项目启动命令。不由 main.py 运行，而是注入到 executor/e2e_tester briefing 中，让 Claude 知道怎么跑项目
- `cleanup.sh` — 杀掉后台进程（如 uvicorn、node），释放文件锁。complete.py 在 merge 前自动调用，失败仅警告不阻塞
- 三者都是模板文件，由 Executor 在开发过程中按需填写
- init.sh、cleanup.sh 内容注入到 executor briefing 中，方便 Executor 判断是否需要填写
- cleanup.sh 内容也注入到 e2e_tester briefing 中，提醒测试后清理进程
...
- `main.py` 入口处 `try/except` 捕获异常并打印恢复提示，改退出逻辑时需保持此兜底
- `.session_lock` 是运行时标记文件，正常退出时删除；改 `sys.exit()` 路径时需确保 lock 被清理
- `.session_lock` 已加入 `.gitignore`，不提交到仓库
- `.cc-dev-framework/session.log` 已加入 `.gitignore`（`*.log` 规则覆盖），不提交到仓库
- 改了 `cleanup.sh` 的模板判断逻辑，需同步检查 `briefing.py` 的 `_read_cleanup_sh()`
- `complete.py` 在 merge 前调用 `cleanup.sh`（`_run_cleanup()`），失败仅警告；之后 `time.sleep(1)` 等 Windows 释放句柄
